...
import { BossShip, BossShip0, BossShip1, BossShip2 } from './objects/BossShip';
...
export class GameManager extends BaseUI{
	...
	container:ObjectManager=new ObjectManager();
	actionC:ActionControl=new ActionControl(this);
	starfield:Starfield;
	player:PlayerShip=new PlayerShip();
	wordInput=new WordInput();
	levelEvents:EventInterpreter;
	...
	addEnemy=(spawnEvent:ISpawnEvent):EnemyShip=>{
	...
	addBoss=(bossType:number):BossShip=>{
		let boss;
		switch (bossType){
