...
export function getLevel(level:number,wpm:number=60):ILevelEvent[]{
...
class MethodPack{
  ...
  wait(distance:number=0):ILevelEvent{
  ...
  ship(type:string,distance:number,x:number,y:number,commands:ICommand[]=null){
    this.setDist(distance);
    // x*=GameUI.VIEW_WIDTH/12;
    // y*=GameUI.VIEW_HEIGHT/12;
    // this.array.push(new SpawnEvent(i,_type,_x,_y,_commands));
    this.array.push({type:EventType.SPAWN,distance:this.distance,spawnEvent:{type,x,y,commands}})
  }

  laser(type:string,distance:number,x:number,y:number,speed:number=0){
  ...
  suicide(_type:string,_distance:number,_x:number,_y:number,_xTo:number,_yTo:number){
    this.ship(_type.charAt(0)+"s"+_type.charAt(1),_distance,_x,_y,[this.move(_xTo,_yTo),this.fire()]);
  }
  
  stealth(_distance:number,_x:number,_y:number,_xTo:number,_yTo:number,_shield:boolean=false){
  ...
  missile(_type:string,_distance:number,_x:number,_y:number,_xTo1:number,_yTo1:number,_xTo2:number=-1,_yTo2:number=-1,_xTo3:number=-1,_yTo3:number=-1){
  ...
  mirror(distance:number=0,index:number=-1){
    this.setDist(distance);
  ...
  offset(_distance:number=0,xOff:number=0,yOff:number=0,index:number=-1){
    this.setDist(_distance);
    ...
    // xOff*=GameUI.VIEW_WIDTH/12;
    // yOff*=GameUI.VIEW_HEIGHT/12;
  ...
  repeat(distance:number,repeatN:number=1,repeatC:number=1,reverse?:boolean){
  ...
  move(x:number,y:number,fire:boolean=false):ICommand{
    // _x*=GameUI.VIEW_WIDTH/12;
    // _y*=GameUI.VIEW_HEIGHT/12;
    return {x,y,fire,move:true,timer:0};
  }
  
  halt(timer:number,fire:boolean=false):ICommand{
    // return new ObjectCommand(GameUI.VIEW_WIDTH/2,GameUI.VIEW_HEIGHT-100,_timer,false,_fire);
    return {x:6,y:12,timer,fire,move:false};
  }
  
  fire():ICommand{
    // return new ObjectCommand(0,0,1,false,true);
    return {timer:1,move:false,fire:true};
  }

  cloneCommand(command:ICommand,xOff:number=0,yOff:number=0):ICommand{
    return {x:command.x+xOff,y:command.y+yOff,timer:command.timer,move:command.move,fire:command.fire};
  }

  mirrorCommand(command:ICommand):ICommand{
    return {x:12-command.x,y:command.y,timer:command.timer,move:command.move,fire:command.fire};
  }

  cloneCommands(commands:ICommand[],xOff:number=0,yOff:number=0):ICommand[]{
    let m:ICommand[]=[];
  ...
  mirrorCommands(commands:ICommand[],xOff:number=0,yOff:number=0):ICommand[]{
    let m:ICommand[]=[];
    for (let i=0;i<commands.length;i++){
