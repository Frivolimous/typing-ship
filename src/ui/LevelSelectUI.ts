...
import { DifficultyPopup } from './windows/DifficultyPopup';
...
export class LevelSelectUI extends BaseUI {
  ...
  private resetLevelStuff = () => {
    ...
    if (recommended) {
      ...
      if (!this.wpmText) {
        this.wpmText = new PIXI.Text('', {fontFamily: Fonts.UI});
      ...
      if (!this.recommendedSuper) {
        this.recommendedSuper = new PIXI.Text('Recommended:', {fontSize: 14, fontFamily: Fonts.UI});
